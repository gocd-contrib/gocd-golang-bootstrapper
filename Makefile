...
go-bootstrapper.darwin.amd64: src/main.go goGet
	GOPATH=$(PWD)/.go GOOS=darwin GOARCH=amd64 go build $(GOFLAGS) -o out/go-bootstrapper-$(VERSION).darwin.amd64 src/main.go
go-bootstrapper.darwin.386: src/main.go goGet
	GOPATH=$(PWD)/.go GOOS=darwin GOARCH=386 go build $(GOFLAGS) -o out/go-bootstrapper-$(VERSION).darwin.386 src/main.go

go-bootstrapper.linux.amd64: src/main.go goGet
	GOPATH=$(PWD)/.go GOOS=linux GOARCH=amd64 go build $(GOFLAGS) -o out/go-bootstrapper-$(VERSION).linux.amd64 src/main.go
